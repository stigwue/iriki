...
/**
* Iriki NoSQL Mongo database engine.
*
*/
class nosql_mongo extends database
{
	/**
    * Database id field.
    ...
    */
	public static function doInitialise($config_values)
	{
		...
		else
		{
			...
			{
			    $conn_string = Self::buildConnString(Self::$_key_values);

				Self::$__instance = new \MongoDB\Driver\Manager($conn_string);
    ...
    */
	public static function doCreate($request)
	{
		...
		else
		{
			...
			//do the write
			$ops = new \MongoDB\Driver\BulkWrite();
			//the following can be called recursively for bulk
			$ops->insert($query);

			$status = $handle->executeBulkWrite($namespace, $ops);
			
			return array(
				'message' => ($status->getInsertedCount() !== 0),
				'data' => (string) $query[Self::ID_FIELD]
			);
		}
	}
    ...
    */
	public static function doUpdate($request)
	{
		...
		else
		{
			...
			//do the update
			$ops = new \MongoDB\Driver\BulkWrite();
			$ops->update($query, array('$set' => $data));
    ...
    */
	public static function doDelete($request)
	{
		...
		else
		{
			...
			//do the update
			$ops = new \MongoDB\Driver\BulkWrite();
			$ops->delete($query, ['limit' => 1]);

			$status = $handle->executeBulkWrite($namespace, $ops);
			
			return ($status->getDeletedCount() !== 0);
		}
	}
}
