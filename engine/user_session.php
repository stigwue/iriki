...
class user_session extends \iriki\engine\request
{
	...
	public function read_by_ip($request, $wrap = true)
	{
	    if (!is_null($request))
		{
			$request->setParameterStatus(array(
			...
			if (count($token_obj) != 0)
			{
				...
				if (isset($token_obj[0]['user_id']))
				{
					...
					if ($token['remember'] == 'true')
					{
						...
						$stamp_expired = ((int)$token['started']) + IRIKI_SESSION_LONG;
					}
					else
					{
						//specified duration after started
	...
	public function invalidate_by_user($request, $wrap = true)
	{
		if (!is_null($request))
		{
			//get the session details first
			...
			if (count($sessions_found) == 0)
			{
				...
				return \iriki\engine\response::error('Sessions not found', $wrap);
			}
			else
			{
				$result = array();
				foreach ($sessions_found as $session)
				{
	...
	public function invalidate_by_ip($request, $wrap = true)
	{
		if (!is_null($request))
		{
	...
	public function invalidate_all($request, $wrap = true)
	{
	    if (!is_null($request))
		{
			$data = $request->getData();
    		...
    		$request->setParameterStatus(array(
				'final' => array('authenticated'),
				'missing' => array(),
				'extra' => array(),
				'ids' => array()
			));

			$sessions_found = $request->read($request, $wrap);
